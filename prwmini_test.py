...
class prwmini_test(imdb):
    def __init__(self, root_dir=None):
        super(prwmini_test, self).__init__('prwmini_test')
    ...
    def image_path_from_index(self, index):
        image_path = osp.join(self.data_path, index)
    ...
    def eval_search(self, gallery_det, gallery_feat, probe_feat, det_thresh=0.5):
        ...
        for i in tqdm(range(len(self._probes))):
            y_true, y_score = [], []
            imgs, rois = [], []
            ...
            probe_imname = self._probes[i][0].split('/')[-1][:-4]
            probe_pid = self._probes[i][2]
    ...
    def load_probes(self):
        ...
        for line_no, line in enumerate(file):
            ...
            query_cam = float(probe_im_name[1])
            probe_roi = probe_roi.astype(np.int32)
            probe_roi[2:] += probe_roi[:2]

            probes.append((self.image_path_from_index(probe_im_name), probe_roi, ID))  # set fixed query box
